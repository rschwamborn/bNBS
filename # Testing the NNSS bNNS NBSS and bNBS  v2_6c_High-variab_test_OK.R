...
# input function
  ...
  fun_high.variab.makes_powerlaw_vector<-  function(alpha, n=100000, seed =  123) {
...
vec1_2 <- fun_high.variab.makes_powerlaw_vector(2)

h1 <- hist(vec1_2,  breaks = log2vec)
plot(log(h1$counts) ~ log(h1$mids))

fun_high.variab.makes_powerlaw_vector<-  function(alpha, n=100000, sd = 0.0004, seed =  123) {
...
vec1_2.high.variab <- fun_high.variab.makes_powerlaw_vector(2, sd = 0.0004)

h1.highvar <- hist(vec1_2.high.variab,  breaks = log2vec)
plot(log(h1.highvar$counts) ~ log(h1.highvar$mids), xlim = c(-11, 2))
...
fun_direct_A_b_CVR.VSR.biomass_input_output_bias <- function(inputalpha, input_body_mass_data_vector) {
  ...
  # the standard log2-binning vector, these are the breaks! ---------------
...
for (w in 1 : n.sim) { 
  ...
  vec.in <- fun_high.variab.makes_powerlaw_vector(alpha.in)
...
summary(output.table1$percent_bias) # high variability setting, skewed inputs
summary(output.table2$percent_bias)# high variability setting, skewed inputs
summary(output.table3$percent_bias)# high variability setting, skewed inputs
...
median(output.table1$SVR_estimate) # 0.04189656 (low var), 1.1187 (high var) 
median (output.table2$SVR_estimate) #  0.152067 (low var), 2.366 (high var) 
# Biomass had approx. 2 to 3 times more variability than A, OK due to added M effect
...
# p-value = 0.007702 ( n = 200, low var.),  p-value = 1.563e-06 (n = 2000, low var.),
# OK p-value = 2.18e-06 (high variability) , OK
...
# calculate percent bias:
percent_bias <- abs(m1 - m2) / m1  * 100
percent_bias # 3.2 percent bias (high var.)
